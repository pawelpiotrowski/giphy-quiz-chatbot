...
module.exports = class Bot extends Conversation {
    ...
    async completion() {
        if(!super.validateResetKeyword(this.cargo)) {
            return Log.ok(['BOT -> Conversation end!']);
        }
        Log.status(['BOT -> Got reset!']);
        await this.outbound.reply('text', super.helpers().ok);
        await this.outbound.typeon();
        await this[_resetQuiz]();
        this.threadKey = '$sequence.welcome';
        await this.outbound.wait(2500);
    ...
    async replyActionResult() {
        ...
        Log.info(['BOT -> Reply action resultThread:', [resultThread.payload]]);
        await this.replyActionSequence(resultThread);
    ...
    async validateActionQuiz() {
        ...
        return (quizValidation.valid) ? this[_updateQuizResult](quizValidation.result) : this.replyActionQuizInvalid();
    }
    ...
    async replyQuiz(quizThread) {
        Log.info(['BOT -> sending quiz question. Question and correct answer are:', [quizThread.thread.intro.payload, quizThread.thread.title.payload, quizThread.thread.correct_answer[0]]])
