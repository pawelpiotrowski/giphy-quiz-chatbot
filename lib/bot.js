...
module.exports = class Bot extends Conversation {
    ...
    async replyThread(thread) {
        ...
        await this.outbound.reply(thread.meta.template, thread.payload, thread.attachment);
    }
    // action quiz
    validateActionQuiz() {
    ...
    async replyActionQuiz() {
        ...
        const questionThread = await quiz.getQuestionThread();
        await this.replyThread(questionThread);
        return false;
    }
    // action setup
