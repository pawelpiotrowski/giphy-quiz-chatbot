...
module.exports = class Bot extends Conversation {
    ...
    // action quiz
    validateActionQuiz() {
        Log.info(['BOT -> Validate quiz reply action']);
    }
    // replyActionQuizHelper
    async replyQuiz(quizThread) {
        await this.outbound.reply(quizThread.title.meta.template, quizThread.title.payload);
        await this.outbound.wait(1500);
        await this.outbound.reply(quizThread.meta.template, quizThread.payload, quizThread.attachment);
    }

    async replyActionQuiz() {
        Log.info(['BOT -> Quiz reply action']);
