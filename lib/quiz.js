...
module.exports = class Quiz {
    ...
    constructor(quizUserData) {
        ...
        this.questionNumber = quizUserData.quiz_question_number;
    ...
    fetchQuestion() {
        ...
        .then(resp => {
            if(resp.data && resp.data.response_code === 0) {
                ...
                qq.question = HtmlEntities.decode(qq.question);
                qq.correct_answer = HtmlEntities.decode(qq.correct_answer);
                _.forEach(qq.incorrect_answers, (answer, ind) => {
                    qq.incorrect_answers[ind] = HtmlEntities.decode(answer);
                });
                return qq;
            }
    ...
    resultModel() {
        return _.cloneDeep(quizModel[this.lang].RESULT);
    }

    questionButtonConvert(model, answer) {
        model.title = answer;
        model.payload = '$quiz.' + _.snakeCase(answer);
        return model;
    }
    ...
    async questionBuilder() {
        ...
        model.intro.payload = _.replace(model.intro.payload, '$number', _.isNumber(this.questionNumber) ? this.questionNumber + 1 : 1);
    ...
    async questionNlpSyntax(questionTitle) {
        Log.info(['QUIZ -> NLP questionTitle ',[ questionTitle ]]);
        const nlpResult = await nlpClient.analyzeSyntax({
            document: { content: questionTitle, type: 'PLAIN_TEXT' }
        });
    ...
    async questionIllustration(questionTitle) {
        ...
        questionNlp = _.isEmpty(questionNlp) ? _.find(config.quiz.categories, { id: Number(this.category) }).slug : questionNlp;
        const illustrations = await giphy.search('gifs', {
            q: questionNlp,
            limit: 5,
            sort: 'relevant'
        });
        const resultSample = _.sample(illustrations.data);
        Log.info(['QUIZ -> NLP',[questionNlp]]);
        Log.info(['QUIZ -> Illustration',[resultSample.images.original.gif_url]]);
        return resultSample.images.original.gif_url;
    }

    async getResultThread() {
        let resultModel = this.resultModel();
        const results = JSON.parse(this.result);
        let resultString = '';
        _.forEach(results, (result, ind) => {
            resultString += (ind + 1) + '. ';
            resultString += (result.correct) ? resultModel.correct : resultModel.incorrect;
            resultString += '\\n';
        });
        resultModel = JSON.stringify(resultModel);
        resultModel = _.replace(resultModel, '$result', resultString);
        return JSON.parse(resultModel);
    }

    async validateAnswer(answer) {
        const question = JSON.parse(this.question);
        Log.info(['QUIZ -> Validate quiz, question is:', [question]]);
        const receivedAnswer = _.isObject(answer) ? answer.payload.split('.')[1] : _.snakeCase(answer);
        Log.info(['QUIZ -> Validate quiz, received answer is:', [receivedAnswer]]);
        const validKeyIndex = _.findIndex(question.valid_keys, vKey => {
            return _.snakeCase(vKey) === receivedAnswer;
        });
        Log.info(['QUIZ -> Validate quiz, validKeyIndex is:', [validKeyIndex]]);
        if(validKeyIndex > -1) {
            const result = {};
            result.answer = _.startCase(receivedAnswer);
            result.correct = (receivedAnswer === _.snakeCase(question.correct_answer[0]));
